...
export const CreateCurrencySchema = z.object({
  ...
  description: z.string().min(1)
    .describe("Currency description (required)"),
  buy_rate: z.number().min(0).optional()
    .describe("Buy exchange rate"),
...
export const UpdateCurrencySchema = z.object({
  code: z.string().length(3)
    .describe("Currency code to update (required)"),
  description: z.string().min(1).optional()
    .describe("New description"),
  buy_rate: z.number().min(0).optional()
    .describe("New buy exchange rate"),
  sell_rate: z.number().min(0).optional()
    .describe("New sell exchange rate"),
  unit: z.number().min(0).optional()
    .describe("New unit amount"),
  response_format: z.nativeEnum(ResponseFormat).default(ResponseFormat.MARKDOWN)
    .describe("Output format: 'markdown' or 'json'")
}).strict();
...
export const DeleteCurrencySchema = z.object({
  code: z.string().length(3)
    .describe("Currency code to delete (required)"),
  response_format: z.nativeEnum(ResponseFormat).default(ResponseFormat.MARKDOWN)
    .describe("Output format: 'markdown' or 'json'")
}).strict();
